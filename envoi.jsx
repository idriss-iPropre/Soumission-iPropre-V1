...
function EnvoiPage({ state, pushToast }) {
  ...
  const buildPrintableHtml = () => {
    ...
    const COLW = { label: '40%', plan: '20%' };

    const sectionsHtml = state.sections.map(sec => {
      const rows = sec.rows.map(r => `
        <tr>
          <td style="padding:9px 12px;border-bottom:1px solid #ededed;width:${COLW.label};vertical-align:middle;font-weight:500">${esc(r.label) || '<span style="color:#bbb">—</span>'}</td>
          ${PLAN_DEFS.map((p, pi) => {
            const isSel = pi === state.selectedPlan;
            const val = r.v[pi] || '—';
            return `<td style="padding:9px 12px;border-bottom:1px solid #ededed;text-align:center;width:${COLW.plan};vertical-align:middle;${isSel?'background:#FFF4DA;font-weight:600;color:#111':'color:#555'}">${esc(val)}</td>`;
          }).join('')}
        </tr>`).join('');
      return `
    ...
    const pricesHtml = `
      <table style="width:100%;border-collapse:collapse;font-size:13px;margin-top:6px;border:1px solid #e5e5e5;table-layout:fixed">
        ...
        <tr style="background:#111;color:#fff">
          ...
          ${PLAN_DEFS.map((p, pi) => {
            const isSel = pi === state.selectedPlan;
            return `<td style="padding:14px 6px;text-align:center;font-family:'Playfair Display',serif;font-size:22px;font-weight:700;${isSel?'background:#F4A51C;color:#1a1208':'background:#fafaf6;color:#333'}">${esc(state.prices[pi] || '—')} $</td>`;
          }).join('')}
        </tr>
        <tr>
...
<div class="doc">
  ...
  const handleSend = (e) => {
    ...
    // Open the PDF in a new window, then open the user's mail client with subject/body.
    openPdfWindow();
    setTimeout(() => {
      const subject = encodeURIComponent(`Soumission iPropre — ${form.company || form.clientName || 'votre entreprise'}`);
      const body = encodeURIComponent(`${form.message}\n\n— Plan sélectionné : ${plan.label}\n— Prix : ${price || '—'} $/mois + tx\n\n(Joindre le PDF téléchargé à ce courriel.)`);
...
Object.assign(window, { EnvoiPage });
